...
for(TIME in  TIMES){
    ...
    WL1=WL1I  #Experimental tesista
    WL2=WL2I #Experimental tesista
    WL3=WL3I #Experimental tesista
    WL4=WL4I #Experimental tesista

    WCL1=WCLI1 #Experimental tesista
    WCL2=WCLI2 #Experimental tesista
    WCL3=WCLI3 #Experimental tesista
    WCL4=WCLI4 #Experimental tesista
    ...
    WSERT = INSW(ZRT-TKL1-TKL2,WSE2,INSW(ZRT-TKL1-TKL2-TKL3,WSE3,WSE4))
    #TERT   =AFGEN(PLMTT,TPS)

    ##EVAPORATION
    ##Explanation  in  section  5.2
    
    WLFL1 = RAIN*(1.0-FRNOF)#sorted
    
    EVSH  = min(EVSC, (WL1*0.0001-WCAD1*TKL1)*1000./DELT+WLFL1)
    #EVSD  = min(EVSC, 0.6*EVSC*(sqrt(DSLR)-sqrt(DSLR-1))+WLFL1)
    EVSD  = min(EVSC, EVSC*(sqrt(DSLR)-sqrt(DSLR-1))+WLFL1)#Experimental tesista
    EVSW  = INSW(DSLR-1.1,EVSH,EVSD)
    EVSWT[IDATE]= EVSW#Experimental tesista

   
    FEVL1 <- max(WL1-WCAD1*TKL1*1.E4,0.)*exp(-EES*(0.25*TKL1))
    ...
    WLFL2 = max(0.,WLFL1-(WCFC1*TKL1*1000.-WL1*0.10)/DELT)
    ...
    WL1   = max(0.,WL1I+(WLFL1-WLFL2-EVSW1)*10.0)
    WL2   = max(0.,WL2I+(WLFL2-WLFL3-EVSW2-TRWL2)*10.0)
    WL3   = max(0.,WL3I+(WLFL3-WLFL4-EVSW3-TRWL3)*10.0)
    WL4   = max(0.,WL4I+(WLFL4-WLFL5-EVSW4-TRWL4)*10.0)
    
    WCUM  = (WL1+WL2+WL3+WL4)/10.0 
    ...
    CKWFL =(WLFL1-EVSW-TRW-WLFL5)*10. 
    CKWIN = WL1-WL1I+WL2-WL2I+WL3-WL3I+WL4-WL4I
...
sahel.graph1 <- ggplot(sahel.data,aes(TIMES,WCLER)) + 
  
    scale_y_continuous("RELATIVE WATER CONTENT LAYER 1", sec.axis=sec_axis(~.*100, name= "RAIN AND TEMPERATURE"))+
    ...
    geom_hline(aes(yintercept  =WCST1), color="red", linetype=5,size=.5) + 
    annotate("text", x =-10, y =WCST1+0.02, label = "WCST", color="red")+
    geom_hline(aes(yintercept  =WCFC1), color="lawngreen", linetype=5,size=.5) +
    annotate("text", x =-10, y =WCFC1+0.02, label = "WCFC", color="lawngreen")+  
    geom_hline(aes(yintercept  =WCWP1), color="yellow4", linetype=5,size=.5) +
    annotate("text", x =-10, y =WCWP1+0.02, label = "WCWP", color="yellow4")+
    geom_hline(aes(yintercept  =WCAD1), color="lightgoldenrod4", linetype=5,size=.5) +
    annotate("text", x =-10, y =WCAD1+0.02, label = "WCAD", color="lightgoldenrod4")+
      
    geom_line(aes(x=TIMES , y  =WCL1T), color="purple",size=1.5)+ 
    annotate("text", x =40, y =1.01, label = "WCL1", color="purple")+
    
    theme(legend.position = "bottom")+
    theme_classic()+
    labs(    x = "DAYS",  
            y = "RELATIVE WATER CONTENT LAYER 1",
    ...
    for (val in sec) {
      sahel.graph1 <- sahel.graph1 + annotate("text",  x =val, y =-0.07, label =SDATES[val], color="grey", angle= 315, size=3)
    }
...
x11(width=16)
print(sahel.graph1,print.eval=TRUE)
...
name <-paste(c(DIR,"/sahel4L ",format(Sys.time(), "%a %b %d %X %Y"),".xls"), collapse = "")
